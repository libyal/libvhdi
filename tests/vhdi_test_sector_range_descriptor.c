/*
 * Library sector_range_descriptor type test program
 *
...
#include "../libvhdi/libvhdi_sector_range_descriptor.h"

#if defined( __GNUC__ ) && !defined( LIBVHDI_DLL_IMPORT )

/* Tests the libvhdi_sector_range_descriptor_initialize function
 * Returns 1 if successful or 0 if not
 */
int vhdi_test_sector_range_descriptor_initialize(
     void )
{
	...
	/* Test regular cases
	 */
	result = libvhdi_sector_range_descriptor_initialize(
	...
	result = libvhdi_sector_range_descriptor_free(
	          &sector_range_descriptor,
	          &error );
	...
	sector_range_descriptor = (libvhdi_sector_range_descriptor_t *) 0x12345678UL;
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libvhdi_sector_range_descriptor_initialize with malloc failing
		 */
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libvhdi_sector_range_descriptor_initialize with memset failing
		 */
...
/* Tests the libvhdi_sector_range_descriptor_free function
 * Returns 1 if successful or 0 if not
 */
int vhdi_test_sector_range_descriptor_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
#endif
{
	...
	VHDI_TEST_RUN(
	 "libvhdi_sector_range_descriptor_initialize",
	 vhdi_test_sector_range_descriptor_initialize );

	VHDI_TEST_RUN(
	 "libvhdi_sector_range_descriptor_free",
	 vhdi_test_sector_range_descriptor_free );
