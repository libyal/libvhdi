/*
 * Library region_table type test program
 *
...
#include "../libvhdi/libvhdi_region_table.h"

#if defined( __GNUC__ ) && !defined( LIBVHDI_DLL_IMPORT )

/* Tests the libvhdi_region_table_initialize function
 * Returns 1 if successful or 0 if not
 */
int vhdi_test_region_table_initialize(
     void )
{
	...
	/* Test regular cases
	 */
	result = libvhdi_region_table_initialize(
	          &region_table,
	          &error );
	...
	result = libvhdi_region_table_free(
	          &region_table,
	          &error );
	...
	region_table = (libvhdi_region_table_t *) 0x12345678UL;
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libvhdi_region_table_initialize with malloc failing
		 */
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libvhdi_region_table_initialize with memset failing
		 */
...
/* Tests the libvhdi_region_table_free function
 * Returns 1 if successful or 0 if not
 */
int vhdi_test_region_table_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
#endif
{
	...
	VHDI_TEST_RUN(
	 ...
	 vhdi_test_region_table_initialize );

	VHDI_TEST_RUN(
	 "libvhdi_region_table_free",
	 vhdi_test_region_table_free );
