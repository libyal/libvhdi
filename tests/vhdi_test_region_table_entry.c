/*
 * Library region_table_entry type test program
 *
...
#include "../libvhdi/libvhdi_region_table_entry.h"

uint8_t vhdi_test_region_table_entry_data1[ 32 ] = {
	0x66, 0x77, 0xc2, 0x2d, 0x23, 0xf6, 0x00, 0x42, 0x9d, 0x64, 0x11, 0x5e, 0x9b, 0xfd, 0x4a, 0x08,
	0x00, 0x00, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x00, 0x01, 0x00, 0x00, 0x00 };

#if defined( __GNUC__ ) && !defined( LIBVHDI_DLL_IMPORT )

/* Tests the libvhdi_region_table_entry_initialize function
 * Returns 1 if successful or 0 if not
 */
int vhdi_test_region_table_entry_initialize(
     void )
{
	...
	/* Test regular cases
	 */
	result = libvhdi_region_table_entry_initialize(
	          &region_table_entry,
	          &error );
	...
	result = libvhdi_region_table_entry_free(
	          &region_table_entry,
	          &error );
	...
	region_table_entry = (libvhdi_region_table_entry_t *) 0x12345678UL;
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libvhdi_region_table_entry_initialize with malloc failing
		 */
		...
		if( vhdi_test_malloc_attempts_before_fail != -1 )
		{
			...
			{
				libvhdi_region_table_entry_free(
				 &region_table_entry,
				 NULL );
			}
		}
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libvhdi_region_table_entry_initialize with memset failing
		 */
...
/* Tests the libvhdi_region_table_entry_free function
 * Returns 1 if successful or 0 if not
 */
int vhdi_test_region_table_entry_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
/* Tests the libvhdi_region_table_entry_read_data function
 * Returns 1 if successful or 0 if not
 */
int vhdi_test_region_table_entry_read_data(
     void )
{
	libcerror_error_t *error                     = NULL;
	libvhdi_region_table_entry_t *region_table_entry = NULL;
	...
	/* Test regular cases
	 */
	result = libvhdi_region_table_entry_read_data(
	          region_table_entry,
	          vhdi_test_region_table_entry_data1,
	          32,
	          &error );
	...
	/* Test libvhdi_region_table_entry_read_data with memcpy failing
	 */
...
#endif
{
	...
	VHDI_TEST_RUN(
	 "libvhdi_region_table_entry_initialize",
	 vhdi_test_region_table_entry_initialize );

	VHDI_TEST_RUN(
	 "libvhdi_region_table_entry_free",
	 vhdi_test_region_table_entry_free );

	VHDI_TEST_RUN(
	 "libvhdi_region_table_entry_read_data",
	 vhdi_test_region_table_entry_read_data );
