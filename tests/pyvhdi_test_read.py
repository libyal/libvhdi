...
def pyvhdi_test_seek_offset_and_read_buffer(
    vhdi_file, input_offset, input_whence, input_size, expected_offset, expected_size
):
...
def pyvhdi_test_read_buffer_at_offset(
    vhdi_file, input_offset, input_size, expected_offset, expected_size
):
...
def pyvhdi_test_read(vhdi_file):
    ...
    if not pyvhdi_test_seek_offset_and_read_buffer(
        vhdi_file, read_offset, os.SEEK_SET, read_size, read_offset, read_size
    ):
        return False
    ...
    if media_size < 1024:
        ...
        if not pyvhdi_test_seek_offset_and_read_buffer(
            vhdi_file, read_offset, os.SEEK_SET, read_size, read_offset, 1024
        ):
            return False
    ...
    if not pyvhdi_test_read_buffer_at_offset(
        vhdi_file, read_offset, read_size, read_offset + read_size, read_size
    ):
        return False
...
def main():
    args_parser = argparse.ArgumentParser(description="Tests read.")
