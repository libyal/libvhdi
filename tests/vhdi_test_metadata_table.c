...
#include "../libvhdi/libvhdi_metadata_table.h"

#if defined( __GNUC__ ) && !defined( LIBVHDI_DLL_IMPORT )

/* Tests the libvhdi_metadata_table_initialize function
 * Returns 1 if successful or 0 if not
 */
int vhdi_test_metadata_table_initialize(
     void )
{
	...
	/* Test regular cases
	 */
	result = libvhdi_metadata_table_initialize(
	          &metadata_table,
	          &error );
	...
	result = libvhdi_metadata_table_free(
	          &metadata_table,
	          &error );
	...
	metadata_table = (libvhdi_metadata_table_t *) 0x12345678UL;
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libvhdi_metadata_table_initialize with malloc failing
		 */
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libvhdi_metadata_table_initialize with memset failing
		 */
...
/* Tests the libvhdi_metadata_table_free function
 * Returns 1 if successful or 0 if not
 */
int vhdi_test_metadata_table_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
#endif
{
	...
	VHDI_TEST_RUN(
	 "libvhdi_metadata_table_initialize",
	 vhdi_test_metadata_table_initialize );

	VHDI_TEST_RUN(
	 "libvhdi_metadata_table_free",
	 vhdi_test_metadata_table_free );
