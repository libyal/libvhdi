      ...
      libcerror_error_t *error )
{
	...
	char extended_format_string[ PYVHDI_ERROR_FORMAT_STRING_SIZE ];
	...
	if( error != NULL )
	{
		...
		if( format_string_length < ( PYVHDI_ERROR_FORMAT_STRING_SIZE - 4 ) )
		{
			     ...
			     PYVHDI_ERROR_STRING_SIZE ) != -1 )
			{
				     ...
				     format_string_length ) != NULL )
				{
					...
					for( error_string_index = 0;
					     error_string_index < PYVHDI_ERROR_STRING_SIZE;
					     error_string_index++ )
					{
